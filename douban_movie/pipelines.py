...
class MongoPipeline(object):
	...
	def open_spider(self, spider):
		...
		self.db = self.client.scrapy.douban_movie
	...
	def process_item(self, item, spider):
		# 更新操作太吃cpu 数据小适用
		# self.db.update_one({'movie_id': item['movie_id']}, {'$set': item}, True)

		# 多条插入会丢失部分数据 应该是插入失败造成的
		self.count += 1
		self.item_list.append(item)
		if self.count >= 100:
			logger.info('insert 100 movie in mongodb')
			try:
				self.db.insert_many(self.item_list).inserted_ids
			except errors.BulkWriteError as e:
				logger.error(e)
				logger.error('=' * 500)
				logger.error([item['title'] for item in self.item_list])
				logger.error('=' * 500)
			finally:
				self.count = 0
				self.item_list = []
